...
dynamic_test.py
=================
DWA / MPPI / RPP icin DINAMIK ORTAM benchmark test scripti.
...
Notlar:
    ...
    - 3 dinamik silindir (h1, h2, h3) navigasyon basladigi an aktive edilir
    - Engel hareketi ana navigasyon dongusunde publish edilir (timer YOK)
"""
...
# ===============================================================
# DINAMIK SILINDIR KOREOGRAFISI
# 3 silindir navigasyon basladigi an aktive edilir
# ===============================================================
ENCOUNTERS = [
    {'name': 'h1', 'type': 'CROSS',     'vel_x': -0.10, 'vel_y': -0.10, 'color': 'KIRMIZI'},
    {'name': 'h2', 'type': 'HEAD-ON',   'vel_x':  0.10, 'vel_y': -0.10, 'color': 'MAVI'},
    {'name': 'h3', 'type': 'REV_CROSS', 'vel_x':  0.10, 'vel_y':  0.00, 'color': 'YESIL'},
]
# ===============================================================
...
class ExperimentNode(Node):
    def __init__(self):
        ...
        # Dinamik silindir publisherlari (3 silindir)
        self.actor_pubs = {
            e['name']: self.create_publisher(Twist, f"/{e['name']}/cmd_vel", 10)
            for e in ENCOUNTERS
        }
        self.active_velocities = {e['name']: None for e in ENCOUNTERS}
    ...
    # ---------- Dinamik silindir kontrolu ----------
    def publish_cmd_vels(self):
        """Aktif engellere hiz komutu gonderir (ana donguden cagrilir, timer YOK)."""
        for name, vel in self.active_velocities.items():
            if vel is not None:
                msg = Twist()
                msg.linear.x = vel[0]
                msg.linear.y = vel[1]
                self.actor_pubs[name].publish(msg)

    def activate_all_actors(self):
        """Tum silindirleri ENCOUNTERS listesindeki hizlarla aktive et."""
        for e in ENCOUNTERS:
            self.active_velocities[e['name']] = (e['vel_x'], e['vel_y'])

    def stop_all_actors(self):
        """Tum silindirleri durdur (sifir hiz)."""
        for name in self.active_velocities:
            self.active_velocities[name] = None
            self.actor_pubs[name].publish(Twist())
...
def main():
    ...
    # Tum dinamik silindirleri aktive et
    experiment_node.activate_all_actors()
    print("Dinamik silindirler aktive edildi:")
    for e in ENCOUNTERS:
        print(f"  {e['name']} ({e['color']}) [{e['type']}] "
              f"v=({e['vel_x']:.2f}, {e['vel_y']:.2f})")
    ...
    while not nav.isTaskComplete():
        ...
        experiment_node.publish_cmd_vels()  # Engel hareketi (timer YOK)
    ...
    # Silindirleri durdur, sonra analiz ve kapanis
    experiment_node.stop_all_actors()
