#!/usr/bin/env bash
# ============================================================================
# run_test_with_bag_dynamic.sh
# ----------------------------------------------------------------------------
# dynamic_test.py ve ros2 bag record'u esgudumlu olarak calistirir.
#
# Kullanim:
#   ./run_test_with_bag_dynamic.sh <ALGORITHM> <SCENARIO>
#
# Cikti:
#   results/<SCENARIO>/<ALGO>_<SCENARIO>_run<N>.csv          (dynamic_test.py)
...
TEST_SCRIPT="$SCRIPT_DIR/dynamic_test.py"
...
# dynamic_test.py'deki ALGORITHM_NAME/SCENARIO_NAME uyumu (uyari amacli)
# Non-greedy regex: ilk tirnak ciftini al (yorum icindeki tirnaklara takilmaz)
...
if [ "$SCRIPT_ALGO" != "$ALGORITHM" ] || [ "$SCRIPT_SCEN" != "$SCENARIO" ]; then
    echo "UYARI: dynamic_test.py icindeki ALGORITHM_NAME=$SCRIPT_ALGO,"
...
# Kayit edilecek topic'ler (dinamik silindirler dahil)
TOPICS=(
    /odom
    /amcl_pose
    /scan
...
# --- dynamic_test.py'yi calistir, TUM stdout+stderr'i log dosyasina yaz ---
echo "dynamic_test.py baslatiliyor..."
echo "============================================================"
...
echo "============================================================"
echo "dynamic_test.py tamamlandi (exit code: $TEST_EXIT_CODE)"
echo ""
echo "Run ozeti:"
echo "  CSV : $RESULTS_DIR/${ALGORITHM}_${SCENARIO}_run${NEXT_RUN}.csv"
echo "  Log : $LOG_PATH"
